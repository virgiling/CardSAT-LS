...
def plot_cdf(
    ...
    for solver_name, solver_info in sorted(results.items()):
        ...
        par2_label = f"{par2:.2f}" if np.isfinite(par2) else "N/A"
        ax.step(
            x_plot,
            y_plot,
            where="post",
            label=f"{display_name} (#Solved: {n_solved}, PAR-2: {par2_label})",
            color=color,
            linestyle="-",
            linewidth=0.9,
            alpha=0.85,
    ...
    if log_scale:
        ax.set_xscale("log")
        ax.set_xlabel("CPU Time (s)", fontsize=16, fontweight="bold")
    else:
        ax.set_xlabel("CPU Time (s)", fontsize=16, fontweight="bold")

    ax.set_ylabel("Solved Instances (SAT)", fontsize=16, fontweight="bold")
    ...
    if all_times:
        ...
        ax.set_xlim(
            left=max(TIME_RESOLUTION_SECONDS, min_time), right=timeout * 1.1
        )
...
cdf_plot_name = "CardSAT-LS-Total"
name_mapping = {
    "cardsat-s20+backbone+pre+swap": "CardSAT-LS+P+B+S",
    "cardsat-s20+false-pre+swap": "CardSAT-LS-P-B+S",
    "cardsat-s20+false+pre-swap": "CardSAT-LS+P-B-S",
    "cardsat-s20+backbone-pre-swap": "CardSAT-LS-P+B-S",
    "cardsat-s20+false-pre-swap": "CardSAT-LS-P-B-S",
}
