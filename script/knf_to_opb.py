...
def opb2knf(input_file):
    ...
    for line in lines:
        ...
        if tokens[0] == "p":
            nVars = int(tokens[2])
            nCls = int(tokens[3])
            print(f"* #variable= {nVars + 1} #constraint= {nCls}")
            if FORMAT == "OPBO":
                print(f"min: +1 x{nVars + 1} ;")
            continue
...
def main():
    parser = argparse.ArgumentParser(description="Convert KNF to OPB format")
    parser.add_argument("-f", "--file", required=True, help="Input KNF file")
    parser.add_argument("-p", "--opbo", action="store_true", help="Use OPBO format")
