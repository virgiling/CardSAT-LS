...
// type 
enum Input_Type {UNKNOWN,CNF,KNF,WCNF,WKNF,WCARD,CAI,LSECNF,WHCNF} ;
  ...
  void Parse(string knf_path, Input_Type input_type) override {
    ...
    while (c != EOF) {
      ...
      if(c == 'k') { // cardinality constraint
        ...
        assert (bound >= 1); // bound at least 1
        SkipWhitespace(file, c);
      } else if (c == 'h') { // hard clause
        statement_type = 'h';
        c = getc_unlocked(file); // skip 'h'
        SkipWhitespace(file, c);
        weight = -1;
        s_weight = "-1";
      } else if (input_type == WHCNF) {
        ParseDouble(file, c, weight, s_weight); // get weight
        assert (weight >= 0); // bound at least 1
        SkipWhitespace(file, c);
        statement_type = 'c';
      ...
      if (input_type != WCARD) {
        ...
        if(statement_type == 'c') { // clause
          ...
          guard = 0;
        } else if (statement_type == 'h') { // hard clause
          ncls++;
          for(auto observer : observers_) {
            observer->CardinalityConstraint(clause, 1, weight, s_weight, guard);
          }
          guard = 0;
        }
    ...
    if (ncls != max_cls && input_type != WHCNF) {
