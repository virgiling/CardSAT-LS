...
Internal::Internal()
    : mode(SEARCH), unsat(false), iterating(false), localsearching(false),
      ...
      reported(false), rephased(0), vsize(0), max_var(0), level(0), vals(0),
      score_inc(1.0), scores(this), conflict(0), ignore(0), propagated(0),
      propagated2(0), best_assigned(0), target_assigned(0),
      no_conflict_until(0), unsat_constraint(false), marked_failed(true),
      proof(0), checker(0), tracer(0), opts(this),
#ifndef QUIET
      profiles(this), force_phase_messages(false),
#endif
      arena(this), prefix("c "), internal(this), external(0),
      cardsat_ls(0), termination_forced(false), vars(this->max_var),
      lits(this->max_var), skip_auxvars(false), ccdclHybridMode(0),
      original_cardinality(0), original_guard(0), cardinality_conflict_literal(0) {
...
void Internal::enlarge(int new_max_var) {
  ...
  enlarge_zero(phases.ls, new_vsize);
...
int Internal::cdcl_loop_with_inprocessing() {
  ...
  //     if (CARcheck_watch(lits[i], c))
  //       num_watches++;
  //   }
  //   LOG("Num of watches %d for cardinality %d", num_watches, c->unwatched -
  //   1);
  // }
  // exit(1);
  ...
  if (skip_auxvars) {
    ...
    if (err) {
      printf("Error parsing auxiliary variables file %s: %s\n", auxvars_file,
             err);
      exit(1);
    }
  }
  ...
  ccdclHybridMode = opts.ccdclMode;
  ...
  while (!res) {
    if (unsat)
      ...
      CARanalyze(); // propagate and analyze
    else if (iterating)
  ...
  //     }
  //     LOG ("Num of watches %d for cardinality %d",num_watches,
  //     c->unwatched-1);
  //   }
...
int Internal::local_search_round(int round) {
  ...
  int res = card_walk_round(limit, true);
...
int Internal::local_search() {
  ...
  if (res == 10) {
    printf("local search determined formula to be satisfiable\n");
...
int Internal::solve(bool preprocess_only) {
  ...
  for (int i = 0; i <= max_var; i++)
    guard_literals.push_back(false);
