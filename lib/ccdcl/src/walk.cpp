...
struct Walker {
  ...
  int constraint_selection;     // (0) weighted random, (1) card first, (2) cls
                                // first
...
int Internal::walk_pick_lit(Walker &walker, Clause *c) {
  ...
  for (const auto lit : *c) {
    ...
    if (val(lit) > 0)
      continue; // skipping true lit (card constraint)
    assert(active(lit));
    propagations++;
  ...
  // Finally add all new unsatisfied (broken) cardinality constraints.
  {
    ...
    for (const auto &w : ws) {
      ...
      if (nSat < d->CARbound() - 1) {
        ...
        // }
      } else { // find replacement watch pointer... (this is why we need to be
               // sorted...)
        ...
        for (replacement_pos = d->CARbound(); replacement_pos < size;
             replacement_pos++) {
    ...
    LOG("broken %" PRId64 " cardinality constraints by flipping %d", broken,
        lit);
    ws.clear();
  }
}
...
inline void Internal::walk_save_minimum(Walker &walker) {
  ...
  if (broken + broken_card >= stats.walk.minimum)
    return;
  VERBOSE(2,
          "new global minimum clauses  %" PRId64
          " cardinality constraints %" PRId64,
          broken, broken_card);
...
int Internal::walk_round(int64_t limit, bool prev) {
  ...
  if (!failed) {
    ...
    for (const auto c : CARclauses) {
      ...
      if (c->guard_literal && val(c->guard_literal) > 0)
        continue;
  ...
  if (!failed) {
    ...
    PHASE("walk", stats.walk.count,
          "starting with %" PRId64 " unsatisfied clauses %" PRId64
    ...
    while (!terminated_asynchronously() &&
      ...
      LOG("now have %" PRId64 " broken clauses %" PRId64
          " broken cardinality constraints",
          broken, broken_card);
      VERBOSE(3,
              "now have %" PRId64 " broken clauses %" PRId64
              " broken cardinality constraints",
              broken, broken_card);
      if (broken + broken_card >= minimum)
        continue;
      minimum = broken + broken_card;
      VERBOSE(2, "new phase minimum %" PRId64 " after %" PRId64 " flips",
              minimum, flips);
      walk_save_minimum(walker);
    }
