...
struct Internal {
  ...
  /*----------------------------------------------------------------------*/
  // CCDCL
  int ccdclHybridMode; // 1 propagate verything (stable mode) 2 = no propagation
                       // on cardinality constraints (unstable mode)
  ...
  vector<Clause *>
      CARclauses; // ordered collection of all original cardinality clauses
  vector<Clause *>
      CARencodingClauses; // ordered collection of all original encoding clauses
  int stable_lim;
  ...
  int split_full_encoding(vector<int> lits, int start, int fresh_var,
                          bool add_derivation);
  void totalizer_branch_clauses(vector<int> left, vector<int> right,
                                vector<int> mid, int bound,
                                bool add_derivation);
  vector<int> totalizer_mid_variables(int left, int right, int bound,
                                      int &fresh_var);
  int totalizer_full_encoding(vector<int> lits, int bound, int fresh_var,
                              bool add_derivation);
  int sort_by_phase(vector<int> &lits);
  void encode_cardinality_constraint(int cidx, bool by_score,
                                     bool add_derivation, bool only_derivation);
  ...
  //
  // If no encoding clauses exist, the native cardinality rows remain the
  // authoritative representation in every Hybrid phase, including mode 2.
  bool native_cardinality_watches_active() const {
    return ccdclHybridMode != 2 || CARencodingClauses.empty();
  }
  void init_watches();
  ...
  int card_walk_round(int64_t limit, bool prev);
  void card_walk_save_minimum();
