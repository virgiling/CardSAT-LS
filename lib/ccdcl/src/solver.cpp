...
int Solver::get_ccdclHybridMode() { return internal->ccdclHybridMode; }

void Solver::connect_cardsat_ls(CardsatLS *cardsat_ls) {
  internal->cardsat_ls = cardsat_ls;
}

void Solver::disconnect_cardsat_ls() { internal->cardsat_ls = nullptr; }
...
void Solver::CARaddGuard(int bound) {
  TRACE("addguard", bound);
  REQUIRE_VALID_STATE();
  REQUIRE_VALID_LIT(bound);
  transition_to_unknown_state();
  external->CARaddGuard(bound);
  adding_clause = bound;
  if (adding_clause)
    STATE(ADDING);
  else if (!adding_constraint)
    STATE(UNKNOWN);
  LOG_API_CALL_END("addguard", bound);
}
