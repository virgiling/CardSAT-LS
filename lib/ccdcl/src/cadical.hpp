...
public:
  ...
  int get_ccdclHybridMode();
  void connect_cardsat_ls(CardsatLS *cardsat_ls);
  void disconnect_cardsat_ls();

  void CARadd(int lit, bool encoding = false);

  void CARaddGuard(int bound);
...
public:
  virtual ~CardsatLS() {};
  virtual void get_phase(std::vector<signed char> &phases) = 0;
  virtual void allocate_memory(int num_vars, int num_clauses) = 0;
  virtual void add_clause(const int *lit, int size, int degree) = 0;
  virtual void init_assignment(const std::vector<signed char> &assignment) = 0;
  virtual int start_cardsat_ls(int limit) = 0;
